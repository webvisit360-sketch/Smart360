...
- Two guest themes per tenant (`Tenant.theme`: mediterran|swipe); load exactly ONE theme CSS (tema-sredozemska/tema-poteg) after tenant fetch. Cover editor fields (cover*) map to CSS vars only the swipe theme CSS reads; in mediterran only coverTitle/coverShowRating apply (reference CSS ignores the vars — do not add CSS). Server clamps cover ranges/enums in the tenant PATCH.
