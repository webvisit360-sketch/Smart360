...
- **meli-pu is a PAYING client, not a test tenant** — call the dev row its "development copy". Content is edited ONLY in the production admin; dev is a read-mostly copy refreshed FROM prod. For edge-case testing create a separate, obviously named test tenant; never edit the dev copy of a real client. Dev must never hold Slovene source text prod lacks (breaks translation-template matching). **Why:** dev drift (b/strong markup, extra subtitle text) broke the EN template alignment (Aug 2026).
- Content normalisation: one-off idempotent sanitiser pass exists as admin action POST /admin/maintenance/normalize-content (button "Normaliziraj oblikovanje besedil" in admin account page) + dev script `scripts/normalize-content.ts`. Translation import + plural import now sanitise values (rich = body*/noteText* via isRichField). sanitizePlain decodes entities to a fixpoint (plain columns store literal "&", never "&amp;") — regression tests in src/tests/sanitize.test.ts.
...
- Admin auth currently uses passkeys, with owner-approved password/recovery hardening queued. The single operator address `pi4.doo@gmail.com` is deliberately separate from public business contact `smart360hq@gmail.com`: the former is the private login identity and security channel; the latter is published in legal/contact surfaces. Do not “tidy” them into one address, because that would expose the console login identifier publicly. Operator seeding may create a row only when `admin_users` is empty, regardless of configured e-mail, so editing the seed constant can never create a second operator. Server-side revocable 30-day sessions use hashed tokens; WebAuthn challenges and 15-minute enrolment tokens are DB-backed and single-use; recovery codes are Argon2id-hashed and one-time. rpID/rpOrigin come from RP_ID/RP_ORIGIN in production and the development domain locally.
