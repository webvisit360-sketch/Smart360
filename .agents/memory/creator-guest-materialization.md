...
**How to apply:** Keep first-approval provenance immutable, update only the guest projection on editorial edits, preserve category attachments from other active proposals, and hide the item only when no active approval remains. Approval, rejection, undo, unapprove, and category moves must be transactional and invalidate the public payload cache. A shared normalized name alone (including a legacy item or unlocated Creator proposal) is not canonical identity: require matching verified OSM identity or matching coordinates, with same-name manual pins allowed a small proximity tolerance against geocoded points. Do not let an archived canonical item silently resurrect through a new place creation; direct operators to its archive.
