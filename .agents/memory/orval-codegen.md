...
- Avoid `format: uuid` with the current generator; use a string pattern and validate UUIDs in the handler.
  **Why:** Orval emits `zod.uuid()`, which is also unavailable in the resolved Zod v3. **How to apply:** new UUID path parameters must not introduce a Zod v4-only call.
