...
export function SearchOverlay({ slug, lang, tenant, isOpen, onClose }: { slug: string, lang: string, tenant?: any, isOpen: boolean, onClose: () => void }) {
    ...
    <div className="mask on" style={{ opacity: 1, pointerEvents: 'auto', display: 'flex', flexDirection: 'column', background: 'var(--wash)' }}>
      <header className="navbar" style={{ background: '#fff', borderBottom: '1px solid var(--line)' }}>
        ...
        <div style={{flex: 1, display: 'flex', alignItems: 'center'}}>
           <input autoFocus value={query} onChange={e => setQuery(e.target.value)} placeholder={t("UI.search.title")} className="searchfield" style={{marginLeft: 12, width: '100%'}} />
        </div>
      </header>
      
      <div className="pagepad" style={{flex: 1, overflowY: 'auto', paddingTop: 20}}>
        {query.length > 2 ? (
          isLoading ? <div className="empty">{t("UI.searching")}</div> :
            ...
            <div className="list">
              {results.map(res => (
                <Link href={buildGuestPath(`/${slug}/c/${res.categoryId}`)} onClick={onClose} className="row" key={res.itemId} style={{flexDirection: 'column', alignItems: 'flex-start', padding: '12px 0'}}>
                  ...
                  <div className="row__t" style={{marginTop: 6}}>{res.title || ''}</div>
                  <div className="card__sub" style={{marginTop: 4}} dangerouslySetInnerHTML={{__html: sanitizeHtml(res.snippet)}}></div>
                </Link>
              ))}
            </div>
          ) : <div className="empty">{t("UI.search.empty")}</div>
        ) : (
          <div className="empty">{t("UI.search.min")}</div>
        )}
      </div>
    </div>
  );
