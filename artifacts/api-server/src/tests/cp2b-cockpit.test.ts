...
test("CP2b owner cockpit: create-by-type, slug freeze, first publish, overview, attribution", async (t) => {
  ...
  await t.test("POST /admin/tenants with type=kamp seeds sections/categories/groups", async () => {
    ...
    const [logRow] = await db
      .select({ detail: changelogTable.detail, actorType: changelogTable.actorType })
      .from(changelogTable)
      .where(eq(changelogTable.tenantId, tenantId));
    assert.equal(logRow!.detail, "tip: kamp");
  ...
  await t.test("slug editable before first publish, frozen after; e-mail fires once", async () => {
    ...
    const setEmail = await jreq(base, "PATCH", `/admin/tenants/${tenantId}`, ownerCookie, {
      email: `cp2b-lastnik-${stamp}@example.com`,
    });
  ...
  await t.test("GET /admin/tenants/:id/changelog attributes owner work", async () => {
    ...
    assert.equal(res.status, 200);
    const rows = (await res.json()) as Array<{ action: string; actorType?: string; createdAt: string }>;
    assert.ok(rows.length >= 3, "create + patches must be logged");
    assert.ok(rows.every((r) => r.actorType === "owner"), "every entry from this session is owner-attributed");
  ...
  // ---------- Hosts see none of this ----------
  await t.test("cockpit endpoints are 404 for a host, even on their own tenant", async () => {
    const [host] = await db
    ...
    for (const [method, path, body] of [
      ...
      ["GET", `/admin/tenants/${tenantId}/changelog`, undefined],
