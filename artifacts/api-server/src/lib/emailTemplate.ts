...
function blockHtml(b: EmailBlock, theme: typeof LEGACY_THEME): string {
  switch (b.kind) {
    case "p":
      return `<p style="font-size:16px;line-height:1.6;color:${theme.body};margin:0 0 14px">${b.segs
        .map(segHtml)
        .join("")}</p>`;
    case "small":
      return `<p style="font-size:13.5px;line-height:1.55;color:${theme.small};margin:0 0 14px">${b.segs
        .map(segHtml)
        .join("")}</p>`;
    case "cta":
       // A real anchor styled with the selected palette. Inline styles
      // styles for broad client support; no tracking redirects — the href is
      // the final destination.
      return (
        `<p style="margin:6px 0 18px">` +
        `<a href="${escHtml(b.url)}" style="display:inline-block;background:${theme.ctaBg};color:${theme.ctaText};text-decoration:none;font-weight:700;font-size:16px;padding:13px 22px;border-radius:12px;font-family:${theme.font}">${escHtml(b.label)}</a>` +
        `</p>`
      );
    case "rows": {
      const trs = b.rows
        .map((r, i) => {
          const border =
            i < b.rows.length - 1 ? `border-bottom:1px solid ${theme.line};` : "";
          return (
            `<tr>` +
            `<td style="${border}padding:11px 15px;color:${MUTED};font-size:14.5px;width:${theme === WELCOME_THEME ? "40%" : "150px"};vertical-align:top;font-family:${theme.font}">${escHtml(r.label)}</td>` +
            `<td style="${border}padding:11px 15px;color:${TITLE_COLOR};font-size:16px;font-weight:700;font-family:${theme.font}">${escHtml(r.value)}</td>` +
            `</tr>`
          );
        })
        .join("");
      return (
        ...
        `style="${theme === WELCOME_THEME ? "table-layout:fixed;overflow-wrap:anywhere;" : ""}border:1px solid ${theme.border};border-radius:12px;border-collapse:separate;margin:6px 0 18px">${trs}</table>`
      );
    }
  }
}
