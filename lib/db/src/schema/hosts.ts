...
/**
 * Single-use account-claim invitations, 72 hours, SHA-256 hash only.
 *
 * Deliberately separate from password resets: an invitation may only set the
 * first password on an unclaimed account, while a reset may only replace a
 * password that already exists. Issuing a new invitation invalidates every
 * older pending invitation for the same account.
 */
export const hostInvitesTable = pgTable(
  "host_invites",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    hostUserId: uuid("host_user_id")
...
/** Audit log of host sign-ins, failures, invites and resets. Passwords are never stored. */
export const hostAuthEventsTable = pgTable("host_auth_events", {
  id: uuid("id").primaryKey().defaultRandom(),
  hostUserId: uuid("host_user_id"),
  // login | login_failed | login_backoff | logout | password_changed |
  // invite_issued | invite_used | reset_requested | reset_completed |
  // account_created | email_changed
...
export type HostInvite = typeof hostInvitesTable.$inferSelect;
