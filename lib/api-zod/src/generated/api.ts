/**
 * Generated by orval v8.23.0 🍺
 * Do not edit manually.
 * Api
 * Smart360 API - multi-tenant guest information PWA
 * OpenAPI spec version: 0.1.0
 */
import * as zod from 'zod';


/**
 * @summary Health check
 */
export const HealthCheckResponse = zod.object({
  "status": zod.string()
})


/**
 * @summary Full published content tree for a tenant (guest view)
 */
export const GetPublicTenantParams = zod.object({
  "slug": zod.coerce.string()
})

export const GetPublicTenantQueryParams = zod.object({
  "lang": zod.coerce.string().optional(),
  "preview": zod.coerce.boolean().optional()
})

export const GetPublicTenantResponse = zod.object({
  "id": zod.string(),
  "slug": zod.string(),
  "customDomain": zod.string().nullish(),
  "name": zod.string(),
  "subtitle": zod.string().nullish(),
  "rating": zod.string().nullish(),
  "reviewsCount": zod.string().nullish(),
  "logoUrl": zod.string().nullish(),
  "heroUrl": zod.string().nullish(),
  "tourUrl": zod.string().nullish(),
  "phone": zod.string().nullish(),
  "whatsapp": zod.string().nullish(),
  "viber": zod.string().nullish(),
  "instagram": zod.string().nullish(),
  "address": zod.string().nullish(),
  "mapQuery": zod.string().nullish(),
  "wifiSsid": zod.string().nullish(),
  "wifiPass": zod.string().nullish(),
  "theme": zod.string(),
  "coverTitle": zod.string().nullish(),
  "coverSubtitle": zod.string().nullish(),
  "coverTitleSize": zod.number(),
  "coverTitleOpacity": zod.number(),
  "coverTextColor": zod.string(),
  "coverSubSize": zod.number(),
  "coverSubOpacity": zod.number(),
  "coverMetaSize": zod.number(),
  "coverMetaOpacity": zod.number(),
  "coverVeil": zod.number(),
  "coverAlign": zod.string(),
  "coverShowRating": zod.boolean(),
  "languages": zod.array(zod.string()),
  "isTemplate": zod.boolean(),
  "isPublished": zod.boolean(),
  "updatedAt": zod.string()
}).and(zod.object({
  "sections": zod.array(zod.object({
  "id": zod.string(),
  "tenantId": zod.string(),
  "key": zod.string(),
  "title": zod.string(),
  "subtitle": zod.string().nullish(),
  "icon": zod.string(),
  "position": zod.number(),
  "isVisible": zod.boolean()
}).and(zod.object({
  "categories": zod.array(zod.object({
  "id": zod.string(),
  "sectionId": zod.string(),
  "label": zod.string(),
  "icon": zod.string(),
  "layout": zod.string(),
  "position": zod.number(),
  "isVisible": zod.boolean()
}).and(zod.object({
  "items": zod.array(zod.object({
  "id": zod.string(),
  "categoryId": zod.string(),
  "title": zod.string().nullish(),
  "body": zod.string().nullish(),
  "price": zod.string().nullish(),
  "priceUnit": zod.string().nullish(),
  "phone": zod.string().nullish(),
  "website": zod.string().nullish(),
  "mapQuery": zod.string().nullish(),
  "difficulty": zod.string().nullish(),
  "duration": zod.string().nullish(),
  "distance": zod.string().nullish(),
  "open24": zod.boolean(),
  "hoursJson": zod.string().nullish().describe('JSON array of 7 entries Mon-Sun, each [openMin, closeMin] in minutes or null when closed; close may pass midnight'),
  "noteType": zod.string().nullish(),
  "noteText": zod.string().nullish(),
  "bullets": zod.array(zod.string()),
  "position": zod.number(),
  "isVisible": zod.boolean(),
  "media": zod.array(zod.object({
  "id": zod.string(),
  "itemId": zod.string().nullish(),
  "tenantId": zod.string().nullish(),
  "url": zod.string(),
  "alt": zod.string().nullish(),
  "position": zod.number()
}))
}))
})))
})))
}))


/**
 * @summary Full-text search across a tenant's visible content
 */
export const SearchPublicTenantParams = zod.object({
  "slug": zod.coerce.string()
})

export const SearchPublicTenantQueryParams = zod.object({
  "q": zod.coerce.string(),
  "lang": zod.coerce.string().optional()
})

export const SearchPublicTenantResponseItem = zod.object({
  "itemId": zod.string(),
  "categoryId": zod.string(),
  "sectionId": zod.string(),
  "title": zod.string(),
  "snippet": zod.string(),
  "categoryLabel": zod.string(),
  "sectionTitle": zod.string()
})
export const SearchPublicTenantResponse = zod.array(SearchPublicTenantResponseItem)


export const AdminLoginBody = zod.object({
  "username": zod.string(),
  "password": zod.string()
})

export const AdminLoginResponse = zod.object({
  "authenticated": zod.boolean()
})


export const AdminLogoutResponse = zod.object({
  "authenticated": zod.boolean()
})


export const GetAdminSessionResponse = zod.object({
  "authenticated": zod.boolean()
})


/**
 * @summary Dashboard counts and recent changes
 */
export const GetAdminOverviewResponse = zod.object({
  "tenantsCount": zod.number(),
  "publishedCount": zod.number(),
  "itemsCount": zod.number(),
  "recentChanges": zod.array(zod.object({
  "id": zod.string(),
  "tenantId": zod.string().nullish(),
  "tenantName": zod.string().nullish(),
  "action": zod.string(),
  "entity": zod.string(),
  "detail": zod.string().nullish(),
  "createdAt": zod.string()
}))
})


export const ListTenantsResponseItem = zod.object({
  "id": zod.string(),
  "slug": zod.string(),
  "customDomain": zod.string().nullish(),
  "name": zod.string(),
  "subtitle": zod.string().nullish(),
  "rating": zod.string().nullish(),
  "reviewsCount": zod.string().nullish(),
  "logoUrl": zod.string().nullish(),
  "heroUrl": zod.string().nullish(),
  "tourUrl": zod.string().nullish(),
  "phone": zod.string().nullish(),
  "whatsapp": zod.string().nullish(),
  "viber": zod.string().nullish(),
  "instagram": zod.string().nullish(),
  "address": zod.string().nullish(),
  "mapQuery": zod.string().nullish(),
  "wifiSsid": zod.string().nullish(),
  "wifiPass": zod.string().nullish(),
  "theme": zod.string(),
  "coverTitle": zod.string().nullish(),
  "coverSubtitle": zod.string().nullish(),
  "coverTitleSize": zod.number(),
  "coverTitleOpacity": zod.number(),
  "coverTextColor": zod.string(),
  "coverSubSize": zod.number(),
  "coverSubOpacity": zod.number(),
  "coverMetaSize": zod.number(),
  "coverMetaOpacity": zod.number(),
  "coverVeil": zod.number(),
  "coverAlign": zod.string(),
  "coverShowRating": zod.boolean(),
  "languages": zod.array(zod.string()),
  "isTemplate": zod.boolean(),
  "isPublished": zod.boolean(),
  "updatedAt": zod.string()
})
export const ListTenantsResponse = zod.array(ListTenantsResponseItem)


export const CreateTenantBody = zod.object({
  "slug": zod.string(),
  "name": zod.string(),
  "subtitle": zod.string().optional(),
  "fromTemplate": zod.boolean().optional()
})

export const CreateTenantResponse = zod.object({
  "id": zod.string(),
  "slug": zod.string(),
  "customDomain": zod.string().nullish(),
  "name": zod.string(),
  "subtitle": zod.string().nullish(),
  "rating": zod.string().nullish(),
  "reviewsCount": zod.string().nullish(),
  "logoUrl": zod.string().nullish(),
  "heroUrl": zod.string().nullish(),
  "tourUrl": zod.string().nullish(),
  "phone": zod.string().nullish(),
  "whatsapp": zod.string().nullish(),
  "viber": zod.string().nullish(),
  "instagram": zod.string().nullish(),
  "address": zod.string().nullish(),
  "mapQuery": zod.string().nullish(),
  "wifiSsid": zod.string().nullish(),
  "wifiPass": zod.string().nullish(),
  "theme": zod.string(),
  "coverTitle": zod.string().nullish(),
  "coverSubtitle": zod.string().nullish(),
  "coverTitleSize": zod.number(),
  "coverTitleOpacity": zod.number(),
  "coverTextColor": zod.string(),
  "coverSubSize": zod.number(),
  "coverSubOpacity": zod.number(),
  "coverMetaSize": zod.number(),
  "coverMetaOpacity": zod.number(),
  "coverVeil": zod.number(),
  "coverAlign": zod.string(),
  "coverShowRating": zod.boolean(),
  "languages": zod.array(zod.string()),
  "isTemplate": zod.boolean(),
  "isPublished": zod.boolean(),
  "updatedAt": zod.string()
})


/**
 * @summary Tenant with full content tree
 */
export const GetTenantParams = zod.object({
  "id": zod.coerce.string()
})

export const GetTenantResponse = zod.object({
  "id": zod.string(),
  "slug": zod.string(),
  "customDomain": zod.string().nullish(),
  "name": zod.string(),
  "subtitle": zod.string().nullish(),
  "rating": zod.string().nullish(),
  "reviewsCount": zod.string().nullish(),
  "logoUrl": zod.string().nullish(),
  "heroUrl": zod.string().nullish(),
  "tourUrl": zod.string().nullish(),
  "phone": zod.string().nullish(),
  "whatsapp": zod.string().nullish(),
  "viber": zod.string().nullish(),
  "instagram": zod.string().nullish(),
  "address": zod.string().nullish(),
  "mapQuery": zod.string().nullish(),
  "wifiSsid": zod.string().nullish(),
  "wifiPass": zod.string().nullish(),
  "theme": zod.string(),
  "coverTitle": zod.string().nullish(),
  "coverSubtitle": zod.string().nullish(),
  "coverTitleSize": zod.number(),
  "coverTitleOpacity": zod.number(),
  "coverTextColor": zod.string(),
  "coverSubSize": zod.number(),
  "coverSubOpacity": zod.number(),
  "coverMetaSize": zod.number(),
  "coverMetaOpacity": zod.number(),
  "coverVeil": zod.number(),
  "coverAlign": zod.string(),
  "coverShowRating": zod.boolean(),
  "languages": zod.array(zod.string()),
  "isTemplate": zod.boolean(),
  "isPublished": zod.boolean(),
  "updatedAt": zod.string()
}).and(zod.object({
  "sections": zod.array(zod.object({
  "id": zod.string(),
  "tenantId": zod.string(),
  "key": zod.string(),
  "title": zod.string(),
  "subtitle": zod.string().nullish(),
  "icon": zod.string(),
  "position": zod.number(),
  "isVisible": zod.boolean()
}).and(zod.object({
  "categories": zod.array(zod.object({
  "id": zod.string(),
  "sectionId": zod.string(),
  "label": zod.string(),
  "icon": zod.string(),
  "layout": zod.string(),
  "position": zod.number(),
  "isVisible": zod.boolean()
}).and(zod.object({
  "items": zod.array(zod.object({
  "id": zod.string(),
  "categoryId": zod.string(),
  "title": zod.string().nullish(),
  "body": zod.string().nullish(),
  "price": zod.string().nullish(),
  "priceUnit": zod.string().nullish(),
  "phone": zod.string().nullish(),
  "website": zod.string().nullish(),
  "mapQuery": zod.string().nullish(),
  "difficulty": zod.string().nullish(),
  "duration": zod.string().nullish(),
  "distance": zod.string().nullish(),
  "open24": zod.boolean(),
  "hoursJson": zod.string().nullish().describe('JSON array of 7 entries Mon-Sun, each [openMin, closeMin] in minutes or null when closed; close may pass midnight'),
  "noteType": zod.string().nullish(),
  "noteText": zod.string().nullish(),
  "bullets": zod.array(zod.string()),
  "position": zod.number(),
  "isVisible": zod.boolean(),
  "media": zod.array(zod.object({
  "id": zod.string(),
  "itemId": zod.string().nullish(),
  "tenantId": zod.string().nullish(),
  "url": zod.string(),
  "alt": zod.string().nullish(),
  "position": zod.number()
}))
}))
})))
})))
}))


export const UpdateTenantParams = zod.object({
  "id": zod.coerce.string()
})

export const UpdateTenantBody = zod.object({
  "slug": zod.string().optional(),
  "customDomain": zod.string().nullish(),
  "name": zod.string().optional(),
  "subtitle": zod.string().nullish(),
  "rating": zod.string().nullish(),
  "reviewsCount": zod.string().nullish(),
  "logoUrl": zod.string().nullish(),
  "heroUrl": zod.string().nullish(),
  "tourUrl": zod.string().nullish(),
  "phone": zod.string().nullish(),
  "whatsapp": zod.string().nullish(),
  "viber": zod.string().nullish(),
  "instagram": zod.string().nullish(),
  "address": zod.string().nullish(),
  "mapQuery": zod.string().nullish(),
  "wifiSsid": zod.string().nullish(),
  "wifiPass": zod.string().nullish(),
  "theme": zod.string().optional(),
  "coverTitle": zod.string().nullish(),
  "coverSubtitle": zod.string().nullish(),
  "coverTitleSize": zod.number().optional(),
  "coverTitleOpacity": zod.number().optional(),
  "coverTextColor": zod.string().optional(),
  "coverSubSize": zod.number().optional(),
  "coverSubOpacity": zod.number().optional(),
  "coverMetaSize": zod.number().optional(),
  "coverMetaOpacity": zod.number().optional(),
  "coverVeil": zod.number().optional(),
  "coverAlign": zod.string().optional(),
  "coverShowRating": zod.boolean().optional(),
  "languages": zod.array(zod.string()).optional(),
  "isTemplate": zod.boolean().optional(),
  "isPublished": zod.boolean().optional()
})

export const UpdateTenantResponse = zod.object({
  "id": zod.string(),
  "slug": zod.string(),
  "customDomain": zod.string().nullish(),
  "name": zod.string(),
  "subtitle": zod.string().nullish(),
  "rating": zod.string().nullish(),
  "reviewsCount": zod.string().nullish(),
  "logoUrl": zod.string().nullish(),
  "heroUrl": zod.string().nullish(),
  "tourUrl": zod.string().nullish(),
  "phone": zod.string().nullish(),
  "whatsapp": zod.string().nullish(),
  "viber": zod.string().nullish(),
  "instagram": zod.string().nullish(),
  "address": zod.string().nullish(),
  "mapQuery": zod.string().nullish(),
  "wifiSsid": zod.string().nullish(),
  "wifiPass": zod.string().nullish(),
  "theme": zod.string(),
  "coverTitle": zod.string().nullish(),
  "coverSubtitle": zod.string().nullish(),
  "coverTitleSize": zod.number(),
  "coverTitleOpacity": zod.number(),
  "coverTextColor": zod.string(),
  "coverSubSize": zod.number(),
  "coverSubOpacity": zod.number(),
  "coverMetaSize": zod.number(),
  "coverMetaOpacity": zod.number(),
  "coverVeil": zod.number(),
  "coverAlign": zod.string(),
  "coverShowRating": zod.boolean(),
  "languages": zod.array(zod.string()),
  "isTemplate": zod.boolean(),
  "isPublished": zod.boolean(),
  "updatedAt": zod.string()
})


export const DeleteTenantParams = zod.object({
  "id": zod.coerce.string()
})

export const DeleteTenantResponse = zod.void()


/**
 * @summary Duplicate a tenant with its full section/category tree
 */
export const DuplicateTenantParams = zod.object({
  "id": zod.coerce.string()
})

export const DuplicateTenantBody = zod.object({
  "slug": zod.string(),
  "name": zod.string(),
  "copyContent": zod.boolean().optional().describe('Copy item contents too; false copies only the section\/category structure')
})

export const DuplicateTenantResponse = zod.object({
  "id": zod.string(),
  "slug": zod.string(),
  "customDomain": zod.string().nullish(),
  "name": zod.string(),
  "subtitle": zod.string().nullish(),
  "rating": zod.string().nullish(),
  "reviewsCount": zod.string().nullish(),
  "logoUrl": zod.string().nullish(),
  "heroUrl": zod.string().nullish(),
  "tourUrl": zod.string().nullish(),
  "phone": zod.string().nullish(),
  "whatsapp": zod.string().nullish(),
  "viber": zod.string().nullish(),
  "instagram": zod.string().nullish(),
  "address": zod.string().nullish(),
  "mapQuery": zod.string().nullish(),
  "wifiSsid": zod.string().nullish(),
  "wifiPass": zod.string().nullish(),
  "theme": zod.string(),
  "coverTitle": zod.string().nullish(),
  "coverSubtitle": zod.string().nullish(),
  "coverTitleSize": zod.number(),
  "coverTitleOpacity": zod.number(),
  "coverTextColor": zod.string(),
  "coverSubSize": zod.number(),
  "coverSubOpacity": zod.number(),
  "coverMetaSize": zod.number(),
  "coverMetaOpacity": zod.number(),
  "coverVeil": zod.number(),
  "coverAlign": zod.string(),
  "coverShowRating": zod.boolean(),
  "languages": zod.array(zod.string()),
  "isTemplate": zod.boolean(),
  "isPublished": zod.boolean(),
  "updatedAt": zod.string()
})


export const CreateSectionParams = zod.object({
  "id": zod.coerce.string()
})

export const CreateSectionBody = zod.object({
  "key": zod.string(),
  "title": zod.string(),
  "subtitle": zod.string().optional(),
  "icon": zod.string(),
  "position": zod.number().optional()
})

export const CreateSectionResponse = zod.object({
  "id": zod.string(),
  "tenantId": zod.string(),
  "key": zod.string(),
  "title": zod.string(),
  "subtitle": zod.string().nullish(),
  "icon": zod.string(),
  "position": zod.number(),
  "isVisible": zod.boolean()
})


export const UpdateSectionParams = zod.object({
  "id": zod.coerce.string()
})

export const UpdateSectionBody = zod.object({
  "key": zod.string().optional(),
  "title": zod.string().optional(),
  "subtitle": zod.string().nullish(),
  "icon": zod.string().optional(),
  "position": zod.number().optional(),
  "isVisible": zod.boolean().optional()
})

export const UpdateSectionResponse = zod.object({
  "id": zod.string(),
  "tenantId": zod.string(),
  "key": zod.string(),
  "title": zod.string(),
  "subtitle": zod.string().nullish(),
  "icon": zod.string(),
  "position": zod.number(),
  "isVisible": zod.boolean()
})


export const DeleteSectionParams = zod.object({
  "id": zod.coerce.string()
})

export const DeleteSectionResponse = zod.void()


export const ReorderSectionsBody = zod.object({
  "ids": zod.array(zod.string())
})

export const ReorderSectionsResponse = zod.object({
  "ok": zod.boolean()
})


export const CreateCategoryParams = zod.object({
  "id": zod.coerce.string()
})

export const CreateCategoryBody = zod.object({
  "label": zod.string(),
  "icon": zod.string(),
  "layout": zod.string(),
  "position": zod.number().optional()
})

export const CreateCategoryResponse = zod.object({
  "id": zod.string(),
  "sectionId": zod.string(),
  "label": zod.string(),
  "icon": zod.string(),
  "layout": zod.string(),
  "position": zod.number(),
  "isVisible": zod.boolean()
})


export const UpdateCategoryParams = zod.object({
  "id": zod.coerce.string()
})

export const UpdateCategoryBody = zod.object({
  "label": zod.string().optional(),
  "icon": zod.string().optional(),
  "layout": zod.string().optional(),
  "position": zod.number().optional(),
  "isVisible": zod.boolean().optional()
})

export const UpdateCategoryResponse = zod.object({
  "id": zod.string(),
  "sectionId": zod.string(),
  "label": zod.string(),
  "icon": zod.string(),
  "layout": zod.string(),
  "position": zod.number(),
  "isVisible": zod.boolean()
})


export const DeleteCategoryParams = zod.object({
  "id": zod.coerce.string()
})

export const DeleteCategoryResponse = zod.void()


export const ReorderCategoriesBody = zod.object({
  "ids": zod.array(zod.string())
})

export const ReorderCategoriesResponse = zod.object({
  "ok": zod.boolean()
})


export const CreateItemParams = zod.object({
  "id": zod.coerce.string()
})

export const CreateItemBody = zod.object({
  "title": zod.string().optional(),
  "body": zod.string().optional(),
  "price": zod.string().optional(),
  "priceUnit": zod.string().optional(),
  "phone": zod.string().optional(),
  "website": zod.string().optional(),
  "mapQuery": zod.string().optional(),
  "difficulty": zod.string().optional(),
  "duration": zod.string().optional(),
  "distance": zod.string().optional(),
  "open24": zod.boolean().optional(),
  "hoursJson": zod.string().nullish(),
  "noteType": zod.string().optional(),
  "noteText": zod.string().optional(),
  "bullets": zod.array(zod.string()).optional(),
  "position": zod.number().optional()
})

export const CreateItemResponse = zod.object({
  "id": zod.string(),
  "categoryId": zod.string(),
  "title": zod.string().nullish(),
  "body": zod.string().nullish(),
  "price": zod.string().nullish(),
  "priceUnit": zod.string().nullish(),
  "phone": zod.string().nullish(),
  "website": zod.string().nullish(),
  "mapQuery": zod.string().nullish(),
  "difficulty": zod.string().nullish(),
  "duration": zod.string().nullish(),
  "distance": zod.string().nullish(),
  "open24": zod.boolean(),
  "hoursJson": zod.string().nullish().describe('JSON array of 7 entries Mon-Sun, each [openMin, closeMin] in minutes or null when closed; close may pass midnight'),
  "noteType": zod.string().nullish(),
  "noteText": zod.string().nullish(),
  "bullets": zod.array(zod.string()),
  "position": zod.number(),
  "isVisible": zod.boolean(),
  "media": zod.array(zod.object({
  "id": zod.string(),
  "itemId": zod.string().nullish(),
  "tenantId": zod.string().nullish(),
  "url": zod.string(),
  "alt": zod.string().nullish(),
  "position": zod.number()
}))
})


/**
 * @summary Update an item; pass categoryId to move it to another category
 */
export const UpdateItemParams = zod.object({
  "id": zod.coerce.string()
})

export const UpdateItemBody = zod.object({
  "categoryId": zod.string().optional(),
  "title": zod.string().nullish(),
  "body": zod.string().nullish(),
  "price": zod.string().nullish(),
  "priceUnit": zod.string().nullish(),
  "phone": zod.string().nullish(),
  "website": zod.string().nullish(),
  "mapQuery": zod.string().nullish(),
  "difficulty": zod.string().nullish(),
  "duration": zod.string().nullish(),
  "distance": zod.string().nullish(),
  "open24": zod.boolean().optional(),
  "hoursJson": zod.string().nullish(),
  "noteType": zod.string().nullish(),
  "noteText": zod.string().nullish(),
  "bullets": zod.array(zod.string()).optional(),
  "position": zod.number().optional(),
  "isVisible": zod.boolean().optional()
})

export const UpdateItemResponse = zod.object({
  "id": zod.string(),
  "categoryId": zod.string(),
  "title": zod.string().nullish(),
  "body": zod.string().nullish(),
  "price": zod.string().nullish(),
  "priceUnit": zod.string().nullish(),
  "phone": zod.string().nullish(),
  "website": zod.string().nullish(),
  "mapQuery": zod.string().nullish(),
  "difficulty": zod.string().nullish(),
  "duration": zod.string().nullish(),
  "distance": zod.string().nullish(),
  "open24": zod.boolean(),
  "hoursJson": zod.string().nullish().describe('JSON array of 7 entries Mon-Sun, each [openMin, closeMin] in minutes or null when closed; close may pass midnight'),
  "noteType": zod.string().nullish(),
  "noteText": zod.string().nullish(),
  "bullets": zod.array(zod.string()),
  "position": zod.number(),
  "isVisible": zod.boolean(),
  "media": zod.array(zod.object({
  "id": zod.string(),
  "itemId": zod.string().nullish(),
  "tenantId": zod.string().nullish(),
  "url": zod.string(),
  "alt": zod.string().nullish(),
  "position": zod.number()
}))
})


export const DeleteItemParams = zod.object({
  "id": zod.coerce.string()
})

export const DeleteItemResponse = zod.void()


export const DuplicateItemParams = zod.object({
  "id": zod.coerce.string()
})

export const DuplicateItemResponse = zod.object({
  "id": zod.string(),
  "categoryId": zod.string(),
  "title": zod.string().nullish(),
  "body": zod.string().nullish(),
  "price": zod.string().nullish(),
  "priceUnit": zod.string().nullish(),
  "phone": zod.string().nullish(),
  "website": zod.string().nullish(),
  "mapQuery": zod.string().nullish(),
  "difficulty": zod.string().nullish(),
  "duration": zod.string().nullish(),
  "distance": zod.string().nullish(),
  "open24": zod.boolean(),
  "hoursJson": zod.string().nullish().describe('JSON array of 7 entries Mon-Sun, each [openMin, closeMin] in minutes or null when closed; close may pass midnight'),
  "noteType": zod.string().nullish(),
  "noteText": zod.string().nullish(),
  "bullets": zod.array(zod.string()),
  "position": zod.number(),
  "isVisible": zod.boolean(),
  "media": zod.array(zod.object({
  "id": zod.string(),
  "itemId": zod.string().nullish(),
  "tenantId": zod.string().nullish(),
  "url": zod.string(),
  "alt": zod.string().nullish(),
  "position": zod.number()
}))
})


export const ReorderItemsBody = zod.object({
  "ids": zod.array(zod.string())
})

export const ReorderItemsResponse = zod.object({
  "ok": zod.boolean()
})


export const AddItemMediaParams = zod.object({
  "id": zod.coerce.string()
})

export const AddItemMediaBody = zod.object({
  "url": zod.string(),
  "alt": zod.string().optional(),
  "position": zod.number().optional()
})

export const AddItemMediaResponse = zod.object({
  "id": zod.string(),
  "itemId": zod.string().nullish(),
  "tenantId": zod.string().nullish(),
  "url": zod.string(),
  "alt": zod.string().nullish(),
  "position": zod.number()
})


export const DeleteMediaParams = zod.object({
  "id": zod.coerce.string()
})

export const DeleteMediaResponse = zod.void()


export const ReorderMediaBody = zod.object({
  "ids": zod.array(zod.string())
})

export const ReorderMediaResponse = zod.object({
  "ok": zod.boolean()
})


export const ListTranslationsQueryParams = zod.object({
  "model": zod.coerce.string(),
  "recordId": zod.coerce.string()
})

export const ListTranslationsResponseItem = zod.object({
  "id": zod.string(),
  "model": zod.string(),
  "recordId": zod.string(),
  "field": zod.string(),
  "lang": zod.string(),
  "value": zod.string()
})
export const ListTranslationsResponse = zod.array(ListTranslationsResponseItem)


/**
 * @summary Create/update a translation; empty value deletes it
 */
export const UpsertTranslationBody = zod.object({
  "model": zod.string(),
  "recordId": zod.string(),
  "field": zod.string(),
  "lang": zod.string(),
  "value": zod.string()
})

export const UpsertTranslationResponse = zod.object({
  "ok": zod.boolean()
})


